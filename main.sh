...
function update_product {
  ...
  if [[ -n "$product_line" ]]; then
    ...
    # Güncellenmiş bilgileri hazırlıyoruz
    local updated_line="$product_name,$new_stock,$new_price,$new_category"
...
function generate_report {
  ...
  case $report_type in
    "Stokta Azalan Ürünler")
      ...
      result=$(awk -F',' -v threshold="$threshold" '$2 < threshold {print $1 ", Stok: " $2 ", Fiyat: " $3 ", Kategori: " $4}' depo.csv)
...
function reset_password {
  ...
  if grep -q ",$username," kullanici.csv; then
    ...
    sed -i "s/^\($username,[^,],[^,],[^,],[^,],[^,],\)[^,]\(,.\)/\1$hashed_password\2/" kullanici.csv
    echo "$(date),Şifre sıfırlama,$username," >> log.csv
